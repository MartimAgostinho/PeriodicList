...
void * exec_node_gen(void * arg){//use threads in the future 
    
    periodic_node  * node = (periodic_node *)arg;
    ...
    while( fn_aux != NULL ){
        ...
        //convert to *(*fnptr)(void *)
        pthread_create(&tid, NULL , fn_aux->fn_ptr , fn_aux->args );
...
void exec_node(periodic_node *node){//use threads in the future 
    ...
    pthread_t tid;
    pthread_create(&tid, NULL , &exec_node_gen ,(void *) node);
